...
This endpoint handles admin registration using the local authentication strategy.
It ensures the user trying to create an admin is the super admin, validates user input, creates a new user account, persists the user data in the database and logs the activity for that user.
...
```json
{
  "detail": "You do not have permission to create admin"
}
```
